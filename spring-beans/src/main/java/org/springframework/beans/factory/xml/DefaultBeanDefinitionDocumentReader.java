...
public class DefaultBeanDefinitionDocumentReader implements BeanDefinitionDocumentReader {
	...
	protected void doRegisterBeanDefinitions(Element root) {
		...
		// 在<beans>此方法中，任何嵌套元素都会导致递归。为了正确传播和保留 <beans> default- 属性，请跟踪当前（父）委托，
		// 该委托可能为 null。出于回退目的，使用对父级的引用创建新的（子）委托，然后最终将 this.delegate 重置回其原始（父级）引用。
		// 此行为模拟一堆委托，但实际上不需要一个。
		BeanDefinitionParserDelegate parent = this.delegate;
		// huang：使用解释器模式，将 Document 解析成 Delegate 对象
